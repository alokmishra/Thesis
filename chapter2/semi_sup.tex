...
of clustering. In order to justify that our results are not accidental, we need to sub-sample the datasets as well as the constraints and then apply the sampled constraints. After this we need to compute the 
variance of cluster stability. We created ten constraints datasets from each constraints dataset by sub-sampling (drawing 90\% of the genes randomly). Then we combined pairs of 
perturbed micro-array and constraints datasets, cluster and then report the Dunn's and Davies Bouldin's clustering quality indices. We repeat this for all pairs. 
...
\includegraphics[scale=0.4]{images_only/semisup/results/analysis/img_stress_ppi/1.eps}
\label{fig:stress_ppi_enrich_1}
}
\subfigure[Section of the image showing significant enrichments]{
\includegraphics[scale=0.35]{images_only/semisup/results/analysis/img_stress_ppi/2.eps}
...
\centering
\subfigure[Section of the image showing significant enrichment]{
\includegraphics[scale=0.4]{images_only/ec2machine/semisup/results/analysis/ccycle_only/1.eps}
