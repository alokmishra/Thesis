...
In Figure-\ref{fig:spirals_cv}, we see that in all the runs, clustering quality improves quickly as the number of known constraints are applied. The cluster similarity (to the original cluster) improves quickly with reducing standard deviation as the number of constraints increases. Goal of semi-supervised clustering is to use external knowledge in the form of known pairwise relations between variables in order to improve the quality of clustering. We have shown that applying more constraints indeed lead to better clustering.
...
Since our technique is quite generic, in future, our work can be extended by using other sources as prior knowledge, for example the similarity between 
