...
data itself, internal indices do not give any indication of the biological significance of the resulting clusters. In the previous section, we also proposed an external 
validation index and used it to show that our clustering algorithm works. But that index is not suitable for the biological validation of results. This is because there 
is an overlap between applying constraints (some of which are justified on the basis of known TF-gene interactions) and counting gene pairs with common parents (BSS).  
...
They devised a figure of merit, z-score, based on mutual information between a clustering result and gene annotation data. The z-score indicates relationships between 
