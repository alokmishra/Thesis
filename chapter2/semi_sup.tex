...
In order to determine the value of $\sigma$ we initially used one of the most popular \textit{internal} cluster quality validation index namely Dunn's Index \citep{Dunn1974wellseparated}. Internal cluster quality validation indices take a dataset and the resulting clustering and use information fully intrinsic to the data itself to assess the quality of clustering. This is different from \textit{external} validation indices that use information independent of the dataset for validating the clustering, e.g. gene ontology. The underlying logic of using this to choose $\sigma$ is to search for a value which results in the best quality clusters. We carried out this $\sigma$ optimization without using the supervision step, clustering only the microarray dataset because our objective is to study the impact of supervision. If we incorporate it prior to optimization then the constraints will impact the original similarity matrix.
...
\begin{figure}[tp]
 \centering
 \includegraphics[scale=1.0]{chapter2/stress_dunn.eps}
 \caption{Stress dataset: Sigma optimization using Dunn's Index}
 \label{fig:sigma_opt_dunn}
\end{figure}

The results for the Dunn's index based sigma optimisation for the stress dataset is shown in Figure-\ref{fig:sigma_opt_dunn} which shows the mean values along with standard deviation error bars. The x-axis uses a log-scale because of the spread of the data.  Dunn's index has the optimal region between 0.001 and 0.03 and has its maximum value (best clustering) at $\sigma=0.007$. It is worthwhile to note that the best quality clustering also has the least standard deviation.

\begin{figure}[tp]
 \centering
 \includegraphics[scale=1.0]{chapter2/stress_davies.eps}
 \caption{Stress dataset: Sigma optimization using Davies Bouldin's Index}
 \label{fig:sigma_opt_dav}
\end{figure}

As seen in Figure-\ref{fig:sigma_opt_dav}, the Davies-Bouldin's index has its optimal region between 0.01 and 0.07. It has its minimum value (best clustering) at $\sigma=0.05$. As the standard deviation at many of the $\sigma$ values in the stress dataset was high and the results of both of these cluster quality validation indices were not completely in agreement ($\sigma=0.007$ vs $\sigma=0.05$), we investigated another independent method for estimating the best $\sigma$. For this, we took a very different approach relating to the use of the constraints. While adding supervision (constraints) we used a value of 1 (maximum similarity) irrespective of the value of $\sigma$ which determines the pairwise similarity values of genes. Now since this value (1) is arbitrary, it may not lead to these constraints being satisfied in the clustering process if the value is \textit{very} different from the rest of the similarity values. However, we would not like these constraint values to be rejected as outliers.  Therefore, we believe that the optimum value of $\sigma$ should also be in the range where the constraints are at least moderately satisfied and not outright rejected. We define the \textit{Constraints Satisfaction Ratio} (CSR) as

\[
\text{CSR}= \left( \frac{\text{number of constraints satisfied}}{\text{number of constraints applied}} \right) 
\]

\begin{figure}[tp]
 \centering
 \includegraphics[scale=1.0]{chapter2/stress_csr.eps}
 \caption{Stress dataset: Sigma optimization using Constraints Satisfaction Ratio}
 \label{fig:sigma_opt_csr}
\end{figure}
 
We used this index to confirm our choice of $\sigma$. As seen in Figure-\ref{fig:sigma_opt_csr}, the best value of sigma is at $\sigma=0.001$. Please note that in this case we did not have to repeat runs of the clustering because as stated earlier, for all supervised matrices, the k-means clustering of the eigenvectors was started from fixed centres. These 50 centres (each representing a cluster) were the genes encoding the TFs that had the highest numbers of DNA-interactions in the DNA-binding dataset. We have not exhaustively searched the space of all possible $\sigma$ values (which can be done with a suitable optimization algorithm but is expensive time-wise), but we have chosen our value based on the consensus of optimal regions across results using all these three validation indices. We have used $\sigma=0.01$ values for all our further analysis for this (stress) dataset. 

\subsubsection{Cell-cycle dataset}

\begin{figure}[tp]
 \centering
 \includegraphics[scale=1.0]{chapter2/ccycle_dunn.eps}
 \caption{Cell-cycle dataset: Sigma optimization using Dunn's Index}
 \label{fig:ccycle_sigma_opt_dunn}
\end{figure}

The result for Dunn's index based optimisation for the cell-cycle dataset is shown in Figure-\ref{fig:ccycle_sigma_opt_dunn} which shows the mean values along with standard deviation error bars. The x-axis uses a log-scale because of the spread of the data.  Dunn's index has its maximum value (best clustering) at $\sigma=0.005$ and has an optimal region from 0.003 to 0.03. 

\begin{figure}[tp]
 \centering
 \includegraphics[scale=1.0]{chapter2/ccycle_davies.eps}
 \caption{Cell-cycle dataset: Sigma optimization using Davies Bouldin's Index}
 \label{fig:ccycle_sigma_opt_dav}
\end{figure}

As seen in Figure-\ref{fig:ccycle_sigma_opt_dav}, the Davies-Bouldin's index has its minimum value (best clustering) at $\sigma=0.03$ and the optimal region between 0.01 to 0.05. Even though the results for this dataset are more in agreement than the stress dataset, they are not in perfect agreement. Therefore, we also investigated using our previously explained \textit{Constraints Satisfaction Ratio}. As seen in Figure-\ref{fig:ccycle_sigma_opt_csr}, the curve shows a steep increase after $\sigma=0.03$. As before the k-means clustering of the eigenvectors was started from fixed centres. These 50 centres (each representing a cluster) were the genes encoding the TFs that had the highest numbers of DNA-interactions in the DNA-binding dataset. Based on consensus of all the three, we have used $\sigma=0.03$ value for all our further analysis for this dataset (cell-cycle). 
 
\begin{figure}[tp]
 \centering
 \includegraphics[scale=1.0]{chapter2/ccycle_csr.eps}
 \caption{Cell-cycle dataset: Sigma optimization using Constraints Satisfaction Ratio}
 \label{fig:ccycle_sigma_opt_csr}
\end{figure}
...
In my original thesis, the techniques were applied on real biological sets. Examiners pointed out that the results that were obtained could be purely by chance because of the characteristics of those particular data-sets and it was dangerous to draw conclusions based on those data-sets alone. It was suggested that data-sets should be perturbed and variance of results should be reported.

In semi-supervised clustering chapter, we are applying constraints in order to improve the quality of clustering. In order to justify that our results are not by accident, we need to re-sample the data-sets and then apply the constraints and then compute the variance of cluster stability. 

We start with computing the cluster consistency of each data-set individually i.e. when we sub-sample them and before integration with otherdatasets, compute the cluster quality consistency across all the sub-sampling runs. The exact procedure is outlined below:

\begin{enumerate}
  
\item We got the full data-sets stress [ref] and cell-cycle [ref] data-sets from SMD [ref]. It had x genes including NORFs. Since not much is known about the function of NORFs , so we removed all the NORF from the data-sets. That left us with x genes in the stress data-set and y genes in the cell-cycle data-sets. They had a and b experiment counts respectively.
 
\item We create ten perturbed data-sets from each of stress and cell-cycle datasets after subsampling (drawing 90\% of the genes randomly).
...
\item Once we have the sigma values, we cluster all the original and perturbed datasets and  compute the Dunn and DB’s index for the resulting clusters. Then we report the mean and std deviation of the indices. 

\item We also compute the biological validity index and report its mean and variance for the datasets.

\item The goal is to show the cluster quality consistency with perturbation in data and clearly demonstrate that the clustering algorithm itself is stable with perturbations in data. Now that we are confident about the algorithm quality, we move on to the next step of studying the semi-supervided version of it.
...
\hline 
\end{tabular}
\caption{Tabulate Db and Dunns values for pertubations of Stress dataset. Report mean and variance. Indicates that algorithm itself results in consistent cluster quality.}
\label{tab:no_constraints}
\end{table}
...
\hline 
\end{tabular}
\caption{Tabulate Db and Dunns values for pertubations of Cell-Cycle dataset. Report mean and variance. Indicates that algorithm itself results in consistent cluster quality.}
\label{tab:no_constraints}
\end{table}
...
Next we demonstrate that the semi-supervised set of experiments too do not produce results randomly and are consistent across sub-sampled datasets. 

1. We create ten constraints data-sets from each constraints dataset by subsampling (drawing 90\% of the genes randomly). 
We, now combine pairs of perturbed, micro-array and constraints data-sets, cluster and then report its Dunn’s, DB and Biological validity index. 
We repeat this for all pairs. This is to demonstrate that our results are not random and the biological index value is not deviating much.
...
\hline 
\end{tabular}
\caption{Tabulate Db and Dunns values for pertubations of Stress-Chip dataset. Report mean and variance. Indicates that algorithm itself results in consistent cluster quality}
\label{tab:no_constraints}
\end{table}
...
\hline 
\end{tabular}
\caption{Tabulate Db and Dunns values for pertubations of Stress-PPI dataset. Report mean and variance. Indicates that algorithm itself results in consistent cluster quality.}
\label{tab:no_constraints}
\end{table}
...
\hline 
\end{tabular}
\caption{Tabulate Db and Dunns values for pertubations of CCycle-Chip dataset. Report mean and variance. Indicates that algorithm itself results in consistent cluster quality.}
\label{tab:no_constraints}
\end{table}
...
\hline 
\end{tabular}
\caption{Tabulate Db and Dunns values for pertubations of CCycle-PPI dataset. Report mean and variance. Indicates that algorithm itself results in consistent cluster quality.}
\label{tab:no_constraints}
\end{table}
...
\hline 
\end{tabular}
\caption{Db and Dunns values for pertubations of CCycle-Yeastract dataset. Indicates that algorithm itself results in consistent cluster quality.}
\label{tab:no_constraints}
\end{table}
...
\hline 
\end{tabular}
\caption{Summary of mean and variance of combined datasets. This shows that our results are not random and small pertubations in data doesn’t change the results of semi-supervised clustering.}
\label{tab:no_constraints}
\end{table}
...
We put the final clusters of genes (before and after combination) through Genomica which is a tool to analyze the characteristics of resulting clustering using Gene Ontology. 
\begin{center}
...
\subsection{Biological Significance using Gene Ontology} \label{biosig_go}
...
The results of mean p-values of all the GO terms' enrichment before and after integration for the stress dataset are shown in Table-\ref{tab:stress:semisup_mean_pvals}. We have calculated the effect of integration of the DNA-binding dataset as we change the number and quality of constraints extracted from it (by changing the p-value threshold below which an interaction is considered a constraint). For the PPI and Yeastract datasets, we took all the interactions that were between the common set of genes between the microarray and the interactions datasets. 

We calculated the mean p-values of enrichment for all the GO terms associated to the genes in individual clusters. But it is possible that the number of clusters is not perfect and some clusters end up having terms that are not much enriched at all. To account for this, we also calculated the mean enrichment of 50 highest enriched GO terms and then compared them before and after integration. Across all the terms, the percentage gain shows the trend that we had seen in Figure-\ref{fig:bss_constraints} for algorithm validation. So, it independently validates that the highest quality constraints are till p-value of 0.0005 and after that even though the number of constraints increases, their quality decreases. This is again in agreement with the observations of the original experimenters of the DNA-binding dataset \citep{harbison04transcriptional}. One thing that is notably different from the results of algorithm validation is that here we see some negative scores relative to the unsupervised one. This could most possibly be explained by the bias induced in the BSS because of counting common parent TFs while the DNA-binding data constraints are also of similar nature. Across the top 50 most enriched terms, the observations are similar though the results are more pronounced (percentage gain is better as compared to across all terms) in most cases. This is ascribed to removal of lots of lowly enriched terms and the noise they were adding. Because of this, we consider the 50 terms enrichment score to be superior.

PPI constraints do not seem to add much value when we see all the terms, but for the top 50 terms, it is improving the results by a small percentage (3.664\%). One of the reasons for this is, as indicated earlier, the integration could be meaningful if each of the datasets complement the other. If there are a lot of conflicting values in the similarities of each of the datasets then the resulting matrix will have a lot of noise and this will be reflected in the final clusters. We observe similar negative results for the Yeastract dataset. While PPI seems like a boundary case, the Yeastract data is definitely not complementary with the stress dataset, hence negative gain across top 50 as well as all the terms. 
